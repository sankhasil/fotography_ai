describe('jarvis progress orb', () => {
  ...
  it('shows the tesla orb + task progress on every prompt, hiding the panel, until the message renders', () => {
    ...
    // progress must reappear and replace the panel until the new reply renders.
    ...
    cy.get('main section.panel-bg .tesla-orb', { timeout: 3000 }).should('exist')
    ...
    cy.get('main section.panel-bg article', { timeout: 60000 }).should('exist')
    cy.get('main section.panel-bg .tesla-orb', { timeout: 60000 }).should('not.exist')
